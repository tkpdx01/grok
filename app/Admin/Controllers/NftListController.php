...
class NftListController extends AdminController
{
    /**
     * Make a grid builder.
     *
    ...
    {
        return Grid::make(new NftList(), function (Grid $grid) {
            $grid->column('id')->sortable();
            $grid->column('name','名称');
            $grid->column('price','价格');
            $grid->column('create_rate','开出概率');
    ...
    {
        return Form::make(new NftList(), function (Form $form) {
            $form->display('id');
            $form->text('name','名称');
            $form->decimal('price','价格');
            $form->rate('create_rate','开出概率');
